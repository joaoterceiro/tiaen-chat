...
export function RAGProvider({ children }: { children: ReactNode }) {
  ...
  const configureEvolution = async (config: EvolutionConfig) => {
    try {
      ...
      // Salvar no Supabase
      await Promise.all([
        supabaseDataService.setSystemSetting('evolution_api_url', config.baseUrl),
        supabaseDataService.setSystemSetting('evolution_api_key', config.apiKey),
        supabaseDataService.setSystemSetting('evolution_instance_name', config.instanceName),
        config.webhook && supabaseDataService.setSystemSetting('evolution_webhook_url', config.webhook)
      ])
